...
class CorrelogramView(ManualClusteringView):
    ...
    def _plot_correlograms(self, ccg):
        ...
        for i, j in self._iter_subplots(n_clusters):
            ...
            self[i, j].lines(pos=[[-self.rp_line/self[i,j].window_size, -1, -self.rp_line/self[i,j].window_size, 1],
                            [self.rp_line/self[i,j].window_size, -1, self.rp_line/self[i,j].window_size, 1]],
                            color=(1, 1, 1, 1))
